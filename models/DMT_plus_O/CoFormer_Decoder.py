...
from models.DMT_plus_O.learner import HPNLearner
import warnings
...
class AnyAttention_2(nn.Module):
    def __init__(self, dim, num_heads, qkv_bias=False):
        super(AnyAttention_2, self).__init__()
        self.norm_q, self.norm_k, self.norm_v = Norm(dim), Norm(dim), Norm(dim)
        self.to_q = nn.Linear(dim, dim, bias=qkv_bias)
        self.to_k = nn.Linear(dim, dim, bias=qkv_bias)
        self.to_v = nn.Linear(dim, dim, bias=qkv_bias)
    ...
    def forward(self, q=None, k=None, v=None, qpos=None, kpos=None, w_mask=None):
        q, k, v = self.get_qkv(q, k, v, qpos, kpos)

        # reshape
        q = rearrange(q, "n b (g c) -> b n g c", g=self.num_heads)
        k = rearrange(k, "n b (g c) -> b n g c", g=self.num_heads)
        v = rearrange(v, "n b (g c) -> b n g c", g=self.num_heads)
        ...
        if w_mask is not None:
            if attn.shape[3] > 1:
                bs = attn.shape[-1]
                attn *= torch.sigmoid(w_mask).view(1, 1, 1, bs)
            if attn.shape[1] > 1:
                bs = attn.shape[1]
                attn *= torch.sigmoid(w_mask).view(1, bs, 1, 1)
...
class Trans_Tokes2Tokes(nn.Module):
    def __init__(
        ...
        self.cross_attn = AnyAttention_2(
            dim=in_channel, num_heads=num_heads
        )
        self.dropout1 = nn.Dropout(dropout)
        self.dropout2 = nn.Dropout(dropout)
...
class Sal_Pred_with_Tok(nn.Module):
    ...
    def forward(self, tok, fea):
        ...
        preds = torch.einsum("q b c, b c h w -> b q h w", self.norm2(tok), fea) * self.scale

        return preds


class Sal_Sub_Pred_with_Tok(nn.Module):
    """
    预测模块
    """
    ...
    def forward(self, tok, fea):
        ...
        if tok.shape[1] == 1:
            tok = tok.expand(-1, fea.shape[0], -1)
        preds = torch.einsum("q b c, b c m n h w -> b q m n h w", self.norm2(tok), fea) * self.scale

        return preds


class AnyAttention_3(nn.Module):
    def __init__(self, dim, out_dim=None, qkv_bias=False):
        super(AnyAttention_3, self).__init__()
        self.norm_q, self.norm_k, self.norm_v = Norm(dim), Norm(dim), Norm(dim)
        out_dim = out_dim if out_dim else dim
        self.to_q = nn.Linear(dim, out_dim, bias=qkv_bias)
        self.to_k = nn.Linear(dim, out_dim, bias=qkv_bias)
        self.to_v = nn.Linear(dim, out_dim, bias=qkv_bias)
    ...
    def get_qkv(self, f):
        q, k, v = self.norm_q(f), self.norm_k(f), self.norm_v(f)
        q, k, v = self.to_q(f), self.to_k(f), self.to_v(f)
        return q, k, v

    def forward(self, f):
        q, k, v = self.get_qkv(f)

        # attn matrix calculation
        attn = torch.einsum("b q c, b k c -> b q k", q, k)
        attn *= self.scale
        attn = F.softmax(attn, dim=-1)
        out = torch.einsum("b q k, b k c -> b q c", attn, v.float())
...
class Weight_Prediction(nn.Module):
    def __init__(self, channel):
        ...
        self.attn_1_1 = AnyAttention_3(dim=16, out_dim=outch1)
        self.attn_1_2 = AnyAttention_3(dim=16, out_dim=outch1)

        self.attn_2_1 = AnyAttention_3(dim=outch1, out_dim=outch2)
        self.attn_2_2 = AnyAttention_3(dim=outch1, out_dim=outch2)

        self.attn_3_1 = AnyAttention_3(dim=outch2, out_dim=outch3)
        self.attn_3_2 = AnyAttention_3(dim=outch2, out_dim=outch3)

        self.norm = nn.LayerNorm(outch3)

        self.linear = nn.Linear(outch3, 1)

        self._init_weight()

    def correlation_calculation(self, query_tokens, suppt_tokens):
        eps = 1e-5

        corrs = []
        for idx, (query_tok, suppt_tok) in enumerate(zip(query_tokens, suppt_tokens)):
            suppt_tok = suppt_tok.squeeze(0)
            nb, ch = suppt_tok.size()
            # suppt_tok = suppt_tok.view(-1, ch)
            suppt_tok = suppt_tok / (suppt_tok.norm(dim=1, p=2, keepdim=True) + eps)

            query_tok = query_tok.squeeze(0)
            na, ch = query_tok.size()
            # query_tok = query_tok.view(-1, ch)
            query_tok = query_tok / (query_tok.norm(dim=1, p=2, keepdim=True) + eps)

            corr = torch.bmm(query_tok.unsqueeze(0), suppt_tok.transpose(0, 1).unsqueeze(0)).squeeze()
            corr = corr.view(na, nb)
            corr = corr.clamp(min=0)
            corrs.append(corr)

        return torch.stack(corrs).permute(1, 2, 0).contiguous()
    ...
    def forward(self, tok):
        split_toks = list(tok.chunk(16, dim=-1))
        tok_corr = self.correlation_calculation(split_toks, split_toks)

        out_1_1 = self.attn_1_1(tok_corr)
        out_1_2 = self.attn_1_2(tok_corr.transpose(0, 1).contiguous())
        tok_fea_1 = out_1_1 + out_1_2.transpose(0, 1).contiguous()
        na, nb, _ = tok_fea_1.shape
        if nb != 1:
            nb_ = round(nb/2)
            tok_fea_1 = F.interpolate(tok_fea_1.permute(2, 0, 1).unsqueeze(0), size=(na, nb_), mode='bilinear', align_corners=False)
            tok_fea_1 = tok_fea_1.squeeze(0).permute(1, 2, 0).contiguous()

        out_2_1 = self.attn_2_1(tok_fea_1)
        out_2_2 = self.attn_2_2(tok_fea_1.transpose(0, 1).contiguous())
        tok_fea_2 = out_2_1 + out_2_2.transpose(0, 1).contiguous()
        na, nb, _ = tok_fea_2.shape
        if nb != 1:
            nb_ = round(nb/2)
            tok_fea_2 = F.interpolate(tok_fea_2.permute(2, 0, 1).unsqueeze(0), size=(na, nb_), mode='bilinear', align_corners=False)
            tok_fea_2 = tok_fea_2.squeeze(0).permute(1, 2, 0).contiguous()

        out_3_1 = self.attn_3_1(tok_fea_2)
        out_3_2 = self.attn_3_2(tok_fea_2.transpose(0, 1).contiguous())
        tok_fea_3 = out_3_1 + out_3_2.transpose(0, 1).contiguous()

        tok_fea_3 = tok_fea_3.mean(dim=1).unsqueeze(0)

        w_pred = self.linear(self.norm(tok_fea_3))
...
class Prototype_Refinement(nn.Module):
    def __init__(self, channel):
        ...
        self.gr_sub_sal_pred = Sal_Sub_Pred_with_Tok(channel)
    ...
    def pred_sigmoid_normalization(self, pred):
        bs, _, size, _, h, w = pred.shape
        pred = torch.sigmoid(pred)
        min_pred = torch.min(pred.view(bs, 1, size, size, -1), dim=4)[0].view(bs, 1, size, size, 1, 1)
        max_pred = torch.max(pred.view(bs, 1, size, size, -1), dim=4)[0].view(bs, 1, size, size, 1, 1)
        norm_pred = (pred - min_pred) / (max_pred - min_pred)
        return norm_pred

    def Weighted_GAP(self, fea, mask):
        b, c, size, _, h, w = fea.shape
        fea = fea * mask
        ratio = h * w / (torch.sum(mask.view(b, 1, size, size, -1), dim=4) + 1e-8)
        # (b, c, size, size)
        vec = ratio * torch.mean(fea.view(b, c, size, size, -1), dim=4)
        return vec

    def split_fea(self, fea, size=1):
        if size == 1:
            return rearrange(fea, "b c h w-> b c 1 1 h w").contiguous()
        b, c, h, w = fea.shape
        kernel_size = h // size + h % size
        stride = int((h - kernel_size) / (size-1))
        padding = 0

        unfold = torch.nn.Unfold(kernel_size=kernel_size, stride=stride, padding=padding)
        unfolded_tensor = unfold(fea).permute(0, 2, 1).reshape(b, size, size, c, kernel_size, kernel_size)
        # (b, c, 3, 3, h’, w‘)
        unfolded_tensor = unfolded_tensor.permute(0, 3, 1, 2, 4, 5)

        return unfolded_tensor

    def forward(self, gr_tokens, com_tokens, fea, w_mask):
        # saliency prediction using group tokens
        N = gr_tokens.shape[1]

        # 切分后的fea
        sub_fea = self.split_fea(fea)
        ...
        gr_sub_preds = self.gr_sub_sal_pred(grs_tgt, sub_fea)
        co_sub_pred, bg_sub_pred, _ = gr_sub_preds.split(1, dim=1)

        # generate prototypes
        co_pred_norm = self.pred_sigmoid_normalization(co_sub_pred)
        bg_pred_norm = self.pred_sigmoid_normalization(bg_sub_pred)

        co_proto = self.Weighted_GAP(sub_fea, co_pred_norm)
        bg_proto = self.Weighted_GAP(sub_fea, bg_pred_norm)
        ds = False
        if fea.shape[2] >= 128:
        ...
        fina_fea = self.hpn_learner(HyperCalFea, w_mask)
        if ds:
            fina_fea = F.interpolate(fina_fea, scale_factor=1/scale, mode="bilinear") + fea
        co_tokens, _ = gr_tokens.split(1, dim=0)
        tgfr_mid_pred = self.com_sal_pred_1(com_tokens, fina_fea)

        return {
                   "co_pred": co_pred,
                   "bg_pred": bg_pred,
                   "com_pred": com_pred,
                   "tgfr_mid_pred": tgfr_mid_pred,
               }, fina_fea
...
class CoFormer_Decoder(nn.Module):
    def __init__(self, cfg):
        ...
        for idx, channel in enumerate(fea_channels):
            ...
            w_mask_pred = Weight_Prediction(hidden_dim)
            self.add_module("w_mask_pred_{}".format(idx + 1), w_mask_pred)
    ...
    def forward(self, features: Dict):
        ...
        x = self.input_proj(x)  # 将输入投影到hidden_dim

        fea_pos = self.pe_layer(x)  # 位置编码
        group_pos = self.group_pos.weight  # 前景/背景嵌入
        com_pos = self.com_pos.weight  # is_Co-Saliency
        ...
        for idx, fea_name in enumerate(self.fea_names[::-1]):
            ...
            co_tgt, bg_tgt = gr_tgt.split(1, dim=0)
            co_pos, bg_pos = group_pos.split(1, dim=0)
            co_tgt = co_tgt + co_pos

            w_mask_pred = getattr(self, "w_mask_pred_{}".format(fea_nums - idx))
            w_mask = w_mask_pred(co_tgt)
            stage_w_masks.append(w_mask)

            parser_layer = getattr(self, "parse_{}".format(fea_nums - idx))  # CoT2T & CtP2T
            tgts = parser_layer(
                group_tgt=gr_tgt,
                com_tgt=com_tgt,
                x=x,
            ...
            if idx != fea_nums - 1:
                ...
                if dec_fea.shape[2] > 16:  # R2R
                    dec_fea = group_att(dec_fea, ds=True, scale=16. / dec_fea.shape[2], w_mask=w_mask)
                else:
                    dec_fea = group_att(dec_fea, w_mask=w_mask)

            gr_tgt = torch.cat([co_tgt, bg_tgt], dim=0)

            if idx != fea_nums - 1 and idx != fea_nums - 2:
                proto_refine_layer = getattr(self, "proto_refine_{}".format(fea_nums - idx))  # TGFR
