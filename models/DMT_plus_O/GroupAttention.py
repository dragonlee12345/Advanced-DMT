...
class AnyAttention(nn.Module):
    ...
    def forward(self, q=None, k=None, v=None, qpos=None, kpos=None, w_mask=None):
        ...
        img_num, q_num, _ = q.shape
        _, k_v_num, _ = k.shape

        # reshape
        q = rearrange(q, "n m (g c) -> (n m) g c", g=self.num_heads)
        k = rearrange(k, "n m (g c) -> (n m) g c", g=self.num_heads)
        v = rearrange(v, "n m (g c) -> (n m) g c", g=self.num_heads)

        # attn matrix calculation
        attn = torch.einsum("q g c, k g c -> q g k", q, k)
        attn *= self.scale
        attn = attn.view(img_num, q_num, self.num_heads, img_num, k_v_num)
        attn *= w_mask.view(img_num, 1, 1, img_num, 1)
        attn = attn.view(img_num*q_num, self.num_heads, img_num*k_v_num)
        attn = F.softmax(attn, dim=-1)
        out = torch.einsum("b q g k, b k g c -> b q g c", attn.unsqueeze(0), v.float().unsqueeze(0))
...
class GroupAttention(nn.Module):
    ...
    def forward(self, feas, ds=False, scale=0.5, w_mask=None):
        bs, _, h, w = feas.shape
        if ds:  # 是否下采样
            ...
            query_feas = rearrange(feas_scale, "b c h w -> b (h w) c")
        else:
            query_feas = rearrange(feas, "b c h w -> b (h w) c")
        pooled_feas = self.msp_block(feas)
        pooled_feas = rearrange(pooled_feas, "b c n -> b n c")
        w_mask = torch.sigmoid(w_mask)
        w_mask = torch.matmul(w_mask.squeeze().view(-1, 1), w_mask.squeeze().view(1, -1))
        attn_out = self.attention(q=query_feas, k=pooled_feas, v=pooled_feas, w_mask=w_mask)
        if ds:
